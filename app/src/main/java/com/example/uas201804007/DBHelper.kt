...
class DBHelper(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    companion object{
        ...
        private val SQL_CREATE_PRODUK = "CREATE TABLE " + DBInfo.ProdukTable.TABLE_NAME + "("+DBInfo.ProdukTable.COL_ID+" VARCHAR(200) PRIMARY KEY, " + DBInfo.ProdukTable.COL_PRODUK + " TEXT, " + DBInfo.ProdukTable.COL_MOTIF + " TEXT, " + DBInfo.ProdukTable.COL_WARNA + " TEXT, " + DBInfo.ProdukTable.COL_SIZE + " TEXT, " + DBInfo.ProdukTable.COL_PRICE + " TEXT)"
    }
    ...
    fun InsertProduk(idin: String, produkin: String, motifin: String, warnain: String, sizein: String, pricein: String) {
        val db = writableDatabase
        val namatable = DBInfo.ProdukTable.TABLE_NAME
        val idt = DBInfo.ProdukTable.COL_ID
        val produkt = DBInfo.ProdukTable.COL_PRODUK
        val motift = DBInfo.ProdukTable.COL_MOTIF
        val warnat = DBInfo.ProdukTable.COL_WARNA
        val sizet = DBInfo.ProdukTable.COL_SIZE
        val pricet = DBInfo.ProdukTable.COL_PRICE
        var sql = "INSERT INTO " + namatable + " (" + idt + ", " + produkt + ", " + motift + "," + warnat + "," + sizet + "," + pricet + ") VALUES('" + idin + "', '" + produkin + "', '" + motifin + "', '" + warnain + "', '" + sizein + "', '" + pricein + "')"
        db.execSQL(sql)
    }
    fun fullDataProduk():ArrayList<DataModelProduk> {
        ...
        if (cursor!!.moveToFirst()){
            while (cursor.isAfterLast==false){
                ...
                produkt = cursor.getString(cursor.getColumnIndex(DBInfo.ProdukTable.COL_PRODUK))
                motift = cursor.getString(cursor.getColumnIndex(DBInfo.ProdukTable.COL_MOTIF))
                warnat = cursor.getString(cursor.getColumnIndex(DBInfo.ProdukTable.COL_WARNA))
                ...
                cakes.add(DataModelProduk(idt, produkt, motift, warnat, pricet))
                cursor.moveToNext()
            }
        }
        return  cakes
    ...
    fun updateProduk(idin: String, produkin: String, motifin: String, warnain: String, sizein: String, pricein: String){
        val db = writableDatabase
        val namatablet = DBInfo.ProdukTable.TABLE_NAME
        val idt = DBInfo.ProdukTable.COL_ID
        val produkt = DBInfo.ProdukTable.COL_PRODUK
        ...
        val pricet = DBInfo.ProdukTable.COL_PRICE
        var sql = "UPDATE "+ namatablet + " SET "+
                produkt+"='"+produkin+"', "+motift+"='"+motifin+"',"+warnat+"='"+warnain+"',"+sizet+"='"+sizein+"',"+pricet+"='"+pricein+"' "+
                "WHERE "+idt+"='"+idin+"'"
        db.execSQL(sql)
    }
}